...
function cabin() {
    alert("I'm sorry but this option is only for premium users, you can go to www.CYOA.com/premium to add a premium subscription");
    let remove = document.getElementById('option1');
    remove.removeEventListener('click', cabin);
}
function countinueCleaing() {
    alert('You have choosen to keep cleaning');
}
function clothes() {
    alert("I'm sorry but that option isn't avaible right now");
    let remove = document.getElementById('option2');
    remove.removeEventListener('click', clothes);
}
